...
# Usage: ./scripts/code-refactor.sh
# Outputs a PR to branch claude/refactor-YYYYMMDD

PROGRESS_LOG="${TASK_PROGRESS_LOG:-$(cd "$(dirname "$0")/.." && pwd)/tasks/logs/code-refactor.progress.md}"
...
# Auto-commit if Claude forgot
if [ -n "$(git status --porcelain 2>/dev/null)" ]; then
  echo "Claude left uncommitted changes — auto-committing..."
  git add -A
  git commit -m "refactor: code quality improvements

Auto-committed by runner (Claude forgot to commit)." --no-verify 2>&1 || true
fi

# Verify CI checks, push branch, and create PR if there are commits
COMMITS=$(git rev-list --count origin/${DEFAULT_BRANCH:-develop}..HEAD 2>/dev/null) || COMMITS=0
