...
@main.route('/api/add-custom-word', methods=['POST'])
@token_required
def add_custom_word(current_user):
    custom_word = request.get_json(force=True)

    current_user_words = current_user.words.all()

    for word in current_user_words:
        word_record = Word.query.filter_by(id=word.word_id).first()
        if word_record.simp == custom_word['simp']:
            res = make_response(jsonify({"message": "ALREADY_IN_BANK"}))
            return res
    ...
    for i, c in enumerate(custom_word['simp']):
        custom_word_char = Word.query.filter_by(simp=c).first()

        if custom_word_char:
            trads.append(custom_word_char.trad)

            if i == 0:
                pinyins.append(custom_word_char.pinyin[:-1])
            elif i == len(custom_word['simp']) - 1:
                pinyins.append(custom_word_char.pinyin[1:])
            else:
                pinyins.append(custom_word_char.pinyin[1:-1])
        else:
            not_found = True
    
    if not_found:
        res = make_response(jsonify({"message": "NOT_FOUND"}))
        return res
    ...
    w = Word(simp=custom_word['simp'], trad=trad, pinyin=pinyin, meaning=custom_word['meaning'])
    ...
    res = make_response(jsonify({"message": "OK", "data": w.as_dict()}))
...
def get_user_words(current_user):
    ...
    user_words_sorted = sorted([uw for uw in user_word_entries if uw], key=lambda x: x.due_date)
